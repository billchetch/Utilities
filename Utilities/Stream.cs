    ...
    public class TCPClientStream : TcpClient, IStream
    {
        ...
        public int Read(byte[] buffer, int offset, int count)
        {
            ...
            catch (System.IO.IOException e)
            {
                ...
                throw new Exception("TCPClientStream::Read throws " + e.Message, e);
            }
        }
    ...
    public StreamFlowController(int localBufferSize, int remoteBufferSize) : this(null, localBufferSize, remoteBufferSize) { }
        ...
        public void Open()
        {
            ...
            try
            {
                ...
                _sendTask = new Task(()=>Send(_sendCancellationTokenSource.Token));
                
                /*while (_sendThread.IsAlive)
                {
                    Thread.Sleep(100);
                }
                _sendThread = new Thread(new ParameterizedThreadStart(Send));
                ...
                //Receive thread
                _receiveTask = new Task(() => Receive(_receiveCancellationTokenSource.Token));
                ...
                //Process thread
                _processTask = new Task(() => Process(_processCancellationTokenSource.Token));
                ...
                //send opening bytes .. tell the remote to reset and let it know this has reset
        ...
        public void Close()
        {
            ...
            //_localReset = false;
            //Reset(false, true);
            Stream.Close(); //12/8/24 MOVED FROM ABOVE!!! Might be a source of errors so look here first
        ...
        protected void Receive(CancellationToken cancellationToken)
        {
            bool rcommand = false;
            bool revent = false;
            bool isData = false;
            ...
            try
            {
                while (Stream.IsOpen && !cancellationToken.IsCancellationRequested)
                {
        ...
        protected void Process(CancellationToken cancellationToken)
        {
            try
            {
        ...
        protected void Send(CancellationToken cancellationToken)
        {
            try
            {
                while (Stream.IsOpen && !cancellationToken.IsCancellationRequested)
